...
class BuildingContract(models.Model):
    _name = 'building.contract'
    _order = 'id desc'

    @api.model
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('building.contract.number') or '/'

        res = super(BuildingContract, self).create(vals)
        return res
    ...
    to_date = fields.Date()
    building_id = fields.Many2one('building.registration',domain=[('state','=','vaccant')])
    lease_amount = fields.Float('Rent Amount')
    no_of_installment = fields.Integer(default=1)
    installment_lines = fields.One2many('installment.lines.details', 'contract_id')
    state = fields.Selection([('draft', 'Draft'),('add installment','Added Installment'), ('confirm', 'Confirm')], default='draft')
    ...
    def confirm(self):
        if len(self.installment_lines) > 0:
            ...
            self.state = 'confirm'
            self.building_id.state = 'occupied'
        else:
            raise UserError('No Inastallents are generated')
        ...
        if self.no_of_installment > 0:
            if self.lease_amount > 0:
                ...
                for line in range(0, self.no_of_installment):
                    self.env['installment.lines.details'].create({
                        'contract_id':self.id,
...
class InstallmentLinesDetails(models.Model):
    _name = 'installment.lines.details'
    _order = 'date asc'
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('installment.number') or '/'

        res = super(InstallmentLinesDetails, self).create(vals)
        return res
    ...
    name = fields.Char(string='Event No', required=True, index=True, copy=False, default='New', readonly=1)
    contract_id = fields.Many2one('building.contract')
    date = fields.Date()
    amount = fields.Float()
    state = fields.Selection([('draft', 'Draft'),('confirmed','Confirmed'), ('pending', 'Pending'),('invoiced','Invoiced')], default='draft')
    from_date = fields.Date()
    to_date = fields.Date()
    customer_id = fields.Many2one('res.partner')
    building_id = fields.Many2one('building.registration')
    ...
    payment_status = fields.Selection([('draft','Draft'),('partial','Partial'),('paid','Paid')],compute='compute_payment')
